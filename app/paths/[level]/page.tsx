...
export default async function PathPage({ params }: { params: Promise<{ level: string }> }) {
  ...
  return (
    <>
      <section className="relative bg-navy-950 py-20 md:py-32 px-5 sm:px-6 overflow-hidden">
        ...
        <div className="relative max-w-2xl mx-auto text-center">
          <h1 className="font-serif text-3xl sm:text-4xl md:text-5xl text-white mb-4 leading-tight">{c.title}</h1>
          <p className="text-navy-200 text-base sm:text-lg mb-8">{c.intro}</p>
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center">
            ...
            <Link
              ...
              className="press inline-flex items-center justify-center px-8 py-4 min-h-[48px] bg-gold-500 text-navy-950 font-semibold rounded-md hover:bg-gold-400 transition-colors"
            >
              Start the {lvl} path →
            </Link>
            <Link
